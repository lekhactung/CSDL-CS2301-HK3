...
--Company Name (của Customer), Freight, SalesTotal = UnitPrice * Quantity*(1-Discount), Percent= Freight/SalesTotal 
	select c.CompanyName, Freight, 'SaleTotal' = sum(od.UnitPrice * od.Quantity * (1-od.Discount)), 'Percent' = Freight /sum(od.UnitPrice * od.Quantity * (1-od.Discount))
...
--30.	Lập danh sách các Order có Order Date trong 1 khoảng thời gian nào đó. Tham số nhập: “From date:” và “To:”. Danh sách gồm: Order ID, Customer, ShipCountry, OrderDate 
	set dateformat dmy
...
--34.	Thông thường các khách hàng muốn biết thông tin về đơn hàng của họ đã đặt hàng vào một ngày nào đó. (Khách hàng sẽ báo tên công ty và ngày đặt hàng). Thông tin gồm tất cả các cột của table Order. Anh chị hãy thiết kế query để thực hiện điều này. 
